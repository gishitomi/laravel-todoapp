...
<div class="container create-card large-card">
    <form action="{{route('tasks.edit', ['id' => $task->id])}}" method="post" class="form-group">
        @csrf
        <div class="input-box">
            ...
            <div class="importance-btns">
                <input type="radio" name="importance" id="high" value="1" {{$task->priority == 1 ? 'checked' : ''}}><label for="high" class="high">高</label>
                <input type="radio" name="importance" id="mid" value="2" {{$task->priority == 2 ? 'checked' : ''}}><label for="mid" class="mid">中</label>
                <input type="radio" name="importance" id="low" value="3" {{$task->priority == 3 ? 'checked' : ''}}><label for="low" class="low">低</label>
            </div>
